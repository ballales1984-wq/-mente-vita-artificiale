...
class Ippocampo(ModuloCerebrale, MemoriaAstratta):
    ...
    def _avvia_consolidamento_automatico(self):
        """
        Avvia thread per consolidamento automatico delle memorie
        Elimina memorie con valenza ≤ 0.5 e importanza ≤ 1.0 dopo 5 minuti
        """
        if not self.consolidamento_attivo:
            self.consolidamento_attivo = True
            self.thread_consolidamento = threading.Thread(
                target=self._worker_consolidamento,
                daemon=True,
                name="ConsolidamentoMemoria"
            )
            self.thread_consolidamento.start()
            print(f"[{self.nome}] 🔄 Sistema consolidamento automatico avviato")
    
    def _worker_consolidamento(self):
        """
        Thread worker che esegue consolidamento periodico
        Gira ogni 5 minuti e pulisce memorie non importanti
        """
        while self.consolidamento_attivo and self.attivo:
            # Aspetta tempo consolidamento
            time.sleep(self.tempo_consolidamento)
            ...
            try:
                eliminate = self.consolida_memorie_intelligente()
                if eliminate > 0:
                    print(f"[{self.nome}] 🗑️  Consolidamento: eliminate {eliminate} memorie non rilevanti")
                    # Salva dopo consolidamento
                    self.salva_su_disco()
            except Exception as e:
                print(f"[{self.nome}] ⚠️ Errore consolidamento: {e}")
    
    def consolida_memorie_intelligente(self) -> int:
        """
        Consolida memorie eliminando quelle con:
        - valenza_emotiva ≤ 0.5 E
        - importanza ≤ 1.0 E
        - età > 5 minuti
        
        Returns:
            int: Numero di memorie eliminate
        """
        ...
        for memoria in self.memoria_episodica:
            ...
            # Criteri per CONSERVARE una memoria:
            # 1. È recente (< 5 minuti)
            # 2. Ha valenza emotiva alta (> 0.5)
            # 3. Ha importanza alta (> 1.0)
            # 4. È stata richiamata più volte (accessi >= 2)
            
            conserva = (
                eta_minuti < 5  # Sempre conserva se recente
                or memoria.valenza_emotiva > self.soglia_valenza_minima
                or memoria.importanza > self.soglia_importanza_minima
                or memoria.accessi >= 2  # Memoria richiamata = importante
            )
            
            if conserva:
                memorie_da_tenere.append(memoria)
            else:
                eliminate += 1
        
        # Aggiorna lista memorie
        self.memoria_episodica = memorie_da_tenere
        self.contatore_consolidamenti += 1
        self.memorie_eliminate += eliminate
        
        return eliminate
    
    def richiama_contestuale(self, contesto_attuale: str, top_k: int = 3) -> Tuple[List[MemoriaEpisodica], Dict[str, Any]]:
        """
        Sistema di richiamo intelligente che cerca memorie simili al contesto attuale
        e genera suggerimenti per influenzare le decisioni
        
        Args:
            contesto_attuale: Descrizione del contesto corrente
            top_k: Numero di memorie da richiamare
            
        Returns:
            Tuple di (memorie_rilevanti, suggerimenti_decisione)
        """
        if not self.memoria_episodica:
            return [], {'suggerimento': 'nessuna_memoria', 'confidence': 0.0}
        
        # Cerca memorie simili per keywords
        contesto_lower = contesto_attuale.lower()
        keywords = set(contesto_lower.split())
        
        memorie_scored = []
        
        for memoria in self.memoria_episodica:
            contenuto_lower = memoria.contenuto.lower()
            
            # Calcola similarity score
            parole_comuni = len(keywords.intersection(set(contenuto_lower.split())))
            
            if parole_comuni > 0:
                # Score composito: similarity + importanza + valenza + accessi + recency
                recency = 1.0 / (1.0 + (time.time() - memoria.timestamp) / 3600)  # decay orario
                
                score = (
                    0.3 * (parole_comuni / len(keywords)) +  # Similarity
                    0.25 * memoria.importanza +  # Importanza
                    0.20 * max(0, memoria.valenza_emotiva) +  # Valenza positiva
                    0.15 * min(1.0, memoria.accessi / 5.0) +  # Accessi normalizzati
                    0.10 * recency  # Recency
                )
                
                memorie_scored.append((score, memoria))
        
        # Ordina per score e prendi top_k
        memorie_scored.sort(key=lambda x: x[0], reverse=True)
        memorie_rilevanti = [mem for _, mem in memorie_scored[:top_k]]
        
        # Aggiorna contatore accessi
        for memoria in memorie_rilevanti:
            memoria.accessi += 1
        
        self.contatore_richiami += len(memorie_rilevanti)
        
        # Genera suggerimenti basati sulle memorie richiamate
        suggerimenti = self._genera_suggerimenti_da_memorie(memorie_rilevanti)
        
        return memorie_rilevanti, suggerimenti
    
    def _genera_suggerimenti_da_memorie(self, memorie: List[MemoriaEpisodica]) -> Dict[str, Any]:
        """
        Genera suggerimenti per influenzare la decisione basandosi sulle memorie richiamate
        
        Args:
            memorie: Lista di memorie rilevanti
            
        Returns:
            Dict con suggerimenti strutturati
        """
        if not memorie:
            return {
                'suggerimento': 'nessuna_memoria',
                'confidence': 0.0,
                'azione_consigliata': None
            }
        
        # Analizza pattern nelle memorie
        valenza_media = sum(m.valenza_emotiva for m in memorie) / len(memorie)
        importanza_media = sum(m.importanza for m in memorie) / len(memorie)
        
        # Estrai azioni passate dal contesto
        azioni_passate = []
        for mem in memorie:
            contesto = mem.contesto
            if 'azione' in contesto:
                azioni_passate.append(contesto['azione'])
        
        # Determina azione consigliata
        azione_consigliata = None
        if azioni_passate:
            # Azione più frequente nelle memorie positive
            from collections import Counter
            azioni_count = Counter(azioni_passate)
            azione_consigliata = azioni_count.most_common(1)[0][0] if azioni_count else None
        
        # Genera suggerimento
        if valenza_media > 0.5:
            suggerimento = 'ripeti_comportamento_positivo'
            confidence = min(0.9, valenza_media * importanza_media)
        elif valenza_media < -0.3:
            suggerimento = 'evita_comportamento_negativo'
            confidence = min(0.8, abs(valenza_media) * importanza_media)
            azione_consigliata = None  # Non ripetere azioni negative
        else:
            suggerimento = 'esplora_nuove_opzioni'
            confidence = 0.5
        ...
        return {
            ...
            'valenza_media': valenza_media,
            'importanza_media': importanza_media,
            'num_memorie': len(memorie)
        }
    ...
    def get_statistiche(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'memorizzazioni_totali': self.contatore_memorizzazioni,
            'richiami_totali': self.contatore_richiami,
            'consolidamenti_eseguiti': self.contatore_consolidamenti,
            'memorie_eliminate': self.memorie_eliminate,
    ...
    def chiudi(self):
        ...
        # Ferma thread consolidamento
        if self.consolidamento_attivo:
            self.consolidamento_attivo = False
            if self.thread_consolidamento and self.thread_consolidamento.is_alive():
                print(f"[{self.nome}] Fermando thread consolidamento...")
                # Aspetta max 2 secondi
                self.thread_consolidamento.join(timeout=2.0)
...
def richiama_contestuale(contesto: str, top_k: int = 3) -> Tuple[List[MemoriaEpisodica], Dict[str, Any]]:
    """
    Richiama memorie contestuali e genera suggerimenti
    
    Args:
        contesto: Descrizione contesto attuale
        top_k: Numero memorie da richiamare
        
    Returns:
        Tuple (memorie_rilevanti, suggerimenti)
    """
    return get_instance().richiama_contestuale(contesto, top_k)

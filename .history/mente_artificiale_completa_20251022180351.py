...
class MenteArtificialeCompleta:
    ...
    def ciclo_cognitivo(self) -> Dict[str, Any]:
        ...
        # Prepara contesto per ragionamento
        contesto = {
            ...
            'biosegnale': onda_percezione.pattern if hasattr(onda_percezione, 'pattern') else ''
        }
        decisione = self.prefrontale.ragiona(contesto)
        ...
        if self.voce and self.voce.attivo:
            # Genera risposta vocale in base all'azione
            risposte_vocali = {
                'avvicinati': 'Mi sto avvicinando',
                'allontanati': 'Mi allontano',
                'esegui_comando': 'Eseguo il comando',
                'cerca_oggetto': f'Sto cercando {risultato_visivo.get("oggetti", [{}])[0].get("classe", "l\'oggetto")}',
                'monitora_ambiente': 'Sto monitorando l\'ambiente',
                'mantieni_distanza': 'Mantengo la distanza di sicurezza'
            }
            
            risposta_vocale = risposte_vocali.get(
                decisione['azione'], 
                decisione.get('risposta', 'Comando ricevuto')
            )
            
            # Aggiungi percezioni se rilevanti
            if risultato_audio['testo']:
                risposta_vocale = f"{risposta_vocale}. {decisione.get('risposta', '')}"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--auto":
        # Modalità automatica senza menu
        print("""
╔══════════════════════════════════════════════════════════════════╗
║                                                                  ║
║        🧠 MENTE ARTIFICIALE COMPLETA v3.0 - AUTO                ║
║                                                                  ║
║    Modalità automatica (5 cicli continui)                       ║
║                                                                  ║
╚══════════════════════════════════════════════════════════════════╝
        """)
